...
public class UserServiceTest {
    ...
    public void setUp() {
        ...
        friends.add(user);
        friends.add(user2);
        when(us.userByName("Ruthger van den Eikhof")).thenReturn(user);
        when(us.createNewUser(user)).thenReturn(Boolean.TRUE);
        when(us.createNewUser(user2)).thenReturn(Boolean.FALSE);
        when(us.allFriends("ruthger_vde")).thenReturn(friends);
        when(us.allUsers()).thenReturn(users);
    }
